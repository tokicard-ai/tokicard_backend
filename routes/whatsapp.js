...
function generateCard() {
  ...
  // ⭐ Zero-width joiner to prevent WhatsApp from splitting numbers
  const zw = "\u200B";
  ...
  const number =
    ...
    random4() + zw +
    random4() + zw +
    random4(); // CLEAN, no breaks in WhatsApp
  ...
  return {
    number, // zero-width protected 16 digits
...
router.post("/", async (req, res) => {
  try {
    ...
    /* ----------------------------------------------------------------------
       ⭐⭐⭐ CARD DETAILS — 2 MESSAGES — ZERO-WIDTH PROTECTED CARD NUMBER
    ---------------------------------------------------------------------- */
    if (userIntent === "card") {
      ...
      // FIRST MESSAGE — no card number here
      await sendMessage(
        from,
      ...
      // SECOND MESSAGE — card number protected inside code block
      await sendMessage(
        from,
        `🔢 *Card Number*\n\`\`\`\n${card.number}\n\`\`\`\n👉 Tap & hold to copy.`,
        []
